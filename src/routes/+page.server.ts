...
import { getNextUp } from '$lib/server/media/next-up';
...
export const load: PageServerLoad = async ({ locals }) => {
	...
	const [recentHistory, myLists, recentMovies, recentShows, [heroItem], nextUp] = await Promise.all(
		[
			...
			// Fallback hero backdrop for when there's no Next Up (nothing in progress
			// yet) — a random library item, re-picked on every load (this `load` reruns
			// per navigation), not cached client-side.
			db
