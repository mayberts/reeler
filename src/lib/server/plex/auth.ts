...
async function authHeaders() {
	const { clientIdentifier } = await getPlexConfig();
	if (!clientIdentifier) throw new Error('Plex client identifier is not configured');
	return {
 ...
 */
export async function buildAuthUrl(pin: PlexPin): Promise<string> {
	const { clientIdentifier } = await getPlexConfig();
	if (!clientIdentifier) throw new Error('Plex client identifier is not configured');
...
export async function getPlexUser(authToken: string): Promise<PlexUser> {
	const response = await fetch(`${PLEX_TV}/api/v2/user`, {
		headers: { ...(await authHeaders()), 'X-Plex-Token': authToken }
	});
	if (!response.ok) {
