 ...
 * isn't a top-level title — also `null` on any unexpected failure (a TMDb error, or a
 * `people`/`credits` table that doesn't exist yet because this deployment hasn't
 * re-applied the schema since these tables were added — cast/crew is a supplementary
 * section, and losing it must never take the rest of the detail page down with it).
 *
...
export async function getOrFetchCredits(mediaItem: MediaItem): Promise<Credits | null> {
	...
	try {
		...
		const fetched = await getTmdbCredits(mediaItem.tmdbId, mediaItem.type);
		...
		console.error('[credits] failed to load/fetch credits', { mediaItemId: mediaItem.id }, err);
		return null;
	}
}
